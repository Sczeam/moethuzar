import Link from "next/link";
import { siteContent } from "@/lib/constants/site-content";

export default function HomeHero() {
  return (
    <section className="h-screen border-b border-sepia-border/60">
      <div className="grid h-full grid-cols-1 lg:grid-cols-[1fr_500px]">
        <div className="relative flex h-full flex-col justify-between px-8 py-8 sm:px-12 sm:py-10 lg:py-12">
          <div className="max-w-4xl">
            <h1 className="mt-2 leading-[0.95] text-ink">
              <span className="font-cooper-display block text-[clamp(2.8rem,7.2vw,5.8rem)] font-black uppercase tracking-[0.025em]">
                MOETHUZAR
              </span>
              <span className="mt-2 block text-[clamp(1.2rem,2.4vw,2rem)] font-semibold tracking-[0.08em]">
                Ready to wear
              </span>
            </h1>
          </div>

          <div className="mb-8 max-w-xl sm:mb-12">
            <p className="text-3xl font-semibold leading-tight text-ink sm:text-4xl">
              Grace in every wear
            </p>
            <p className="mt-4 text-lg text-charcoal">
              Cash on delivery across Myanmar. Timeless silhouettes for daily
              wear.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="#latest-products" className="btn-primary">
                Browse Products
              </Link>
              <Link href="/order/track" className="btn-secondary">
                Track Order
              </Link>
            </div>
          </div>
        </div>

        <div className="relative mx-6 my-8 mr-0 h-[calc(100%-4rem)] border border-sepia-border/70 bg-paper-light lg:my-12 lg:ml-10 lg:mr-[50px] lg:h-[calc(100%-6rem)]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={siteContent.heroImageUrl}
            alt="Moethuzar hero"
            className="h-full w-full object-cover"
          />
          <div className="absolute inset-0 bg-teak-brown/30 mix-blend-multiply" />
          <div className="absolute inset-0 border border-antique-brass/30" />
        </div>
      </div>
    </section>
  );
}
